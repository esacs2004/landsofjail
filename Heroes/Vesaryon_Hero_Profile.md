# Vesaryon, the Annihilator

Last updated: 2026-06-25
Source: images/Vesaryon/ screenshots provided in this workspace

## 1) Hero Overview
- Hero name: Vesaryon
- Title shown: the Annihilator
- Rarity shown: SR
- Hero type: Shieldbearer (shield icon)
- Minions shown: 9
- Troop Capacity shown: 13,577
- Status message shown: The Hero is in the training camp and cannot get upgraded.

## 2) Total Hero Stats Snapshot (Observed)

From screenshot: 2026-06-25 070336

### Arrest totals
| Stat | Value |
|---|---:|
| Hero ATK | 1,827 |
| Hero DEF | 2,754 |
| Hero HP | 36,205 |
| Minion ATK | 604 |
| Minion DEF | 918 |
| Minion HP | 12,087 |

### War totals
| Stat | Value |
|---|---:|
| Shieldbearer ATK | 42.39% |
| Shieldbearer DEF | 42.39% |
| Shieldbearer Lethality | 65.77% |
| Shieldbearer HP | 68.4% |

## 3) Skills (Hero Panel)

Observed levels on the panel: Arrest skills at Lv.1, Lv.1, Lv.1 (third shown as locked slot); War/development skills at Lv.1 and Lv.1.

### Arrest skills
| Skill | Base effect shown | Level progression shown |
|---|---|---|
| Apocalyptic Barrage | Uses Machine Gun to barrage, dealing DMG equal to ATK*260% to all enemies within range. | AoE DMG Increase: 260% / 280% / 300% / 320% / 340% |
| Explosive Blast | Launches a missile, dealing AoE DMG equal to ATK*160% to the hit target. | AoE DMG Increase: 160% / 175% / 190% / 205% / 220% |
| Annihilation Mode | Enters Annihilation Mode when HP drops below 60%, increasing ATK Speed by 16% and converting basic attack's DMG to AoE DMG. | ATK Speed Increase: 16% / 18% / 20% / 22% / 24% |

Observed unlock text on Arrest panel:
- Unlock Requires: 2-Star Hero

### War/development skills
| Skill | Base effect shown | Level progression shown |
|---|---|---|
| Armored Intimidation | Reduces enemy DMG by 4%. | Enemy DMG Reduction: 4% / 8% / 12% / 16% / 20% |
| Rapid Recruitment | Increases Recruitment Point recovery speed by 3%. | Recruitment Points Bonus: 3% / 6% / 9% / 12% / 15% |

Observed upgrade-state text on War panels:
- Upgrade button shown with shard progress: 97/15.

## 4) Exclusive Equipment

Not available in provided Vesaryon UI evidence.

Evidence from provided screenshots:
- Bottom navigation shows Stats and Skills tabs only, with no Exclusive Equipment tab.

## 5) Coverage Status

Confirmed from screenshots:
- Hero overview values and rarity.
- Total Hero Stats panel values (Arrest + War).
- Names and effects, with progression rows for all visible Vesaryon skills.
- Arrest unlock requirement text for Annihilation Mode panel.
- No Exclusive Equipment tab shown on Vesaryon panel.

Inferred/high-confidence:
- None beyond directly displayed values.

Source limitations:
- None for the fields documented in this profile.
